...
/*
ENDPOINT : /subcategory/:id_subcategory"
FRONT : 'categoria/id_category?subcategory=1' // Si no se entrega la subcategory darla por default
router.route("/subcategory/:id_subcategory").get(getProductsByCategories)
*/
const getProductsBySubcategory = dryFn(async(req, res, next)=> {
   ...
   if(req.query.sort && req.query.sort == 'name'){
      objQuery = { order  : [req.query.sort, "ASC"] }
   };

   const totalRows = await Product.count({
      where : {
         fk_subcategory : req.params.id_subcategory
      }
   });
  ...
  const products = await Product.findAll({ where : {fk_subcategory : req.params.id_subcategory}, ...objQuery, include: [{ model: Subcategory, include : {model : Category} }] });
